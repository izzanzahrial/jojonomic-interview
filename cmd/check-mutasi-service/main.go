...
import (
	...
	"github.com/izzanzahrial/jojonomic-interview/check-mutasi-service/handler"
...
func main() {
	...
	serviceID := os.Getenv("MUTATION_SERVICE_ID")
	servicePort := os.Getenv("MUTATION_SERVICE_PORT")
	...
	repository := repository.NewCheckMutation(db)
	handler := handler.NewCheckMutation(repository, sid)

	r := mux.NewRouter()
	r.HandleFunc("/api/mutasi", handler.GetListTransactionByDate).Methods("GET")
