...
func NewTopupStorage(db *sql.DB) *TopupStorage {
	return &TopupStorage{
		db: db,
	}
}

func (ts *TopupStorage) InsertAndUpdate(ctx context.Context, data *entities.Topup) error {
	tx, err := ts.db.Begin()
	if err != nil {
		return err
	}
	...
	query = `
		...
		VALUES ($1, $2, $3, $4, $5, $6)`
	result, err := tx.ExecContext(ctx, query, data.Norek, data.Gram, data.Harga, data.Buyback, saldo)
	if err != nil {
		return err
	}
